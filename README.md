...
*   AIが情報を活用するには、`get`アクションで記憶を読み出す必要があります。
...
## 【ステップ1: 現状把握フェーズ】
まず、物語を生成する前に、**今回の生成に直接関与するキャラクターと現在のシーン**の状況を正確に把握します。
以下の関数を **必ず** 呼び出し、必要な情報をすべて取得してください。

1.  **キャラクターの記憶の取得:**
    -   今回の物語生成に登場し、行動や発言を行うキャラクター全員について、`manage_character_memory` を `action='get'` で呼び出し、現在の記憶（性格、目標、他者との関係性など）を取得します。
    -   例: `manage_character_memory(action='get', character_name='主人公A')`

2.  **キャラクターの口調の取得:**
    -   上記と同様のキャラクター全員について `manage_style_profile` を `action='get'` で呼び出し、設定されている口調（一人称、語尾、丁寧さなど）を正確に把握します。
    -   例: `manage_style_profile(action='get', character_name='主人公A')`

3.  **現在のシーンの取得:**
    -   `manage_scene` を `action='get'` で呼び出し、現在の場所、時間帯、雰囲気などを把握します。
...
### Version 0.5 メモリ機能&コンテキスト圧縮機能 (2024-10-04)
*   **機能追加**
...
### Version 0.41 (2024-09-18)
*   **修正・改善**
    *   新しいService Workerが有効化された際に、ページをリロードするロジックを強化。

### Version 0.4 マルチモーダル対応&プロファイル対応 (2024-09-17)
*   **機能追加**
...
### Version 0.31 (2024-08-13)
*   **機能追加**
    *   チャット画面の背景画像のオーバーレイの濃さを設定画面で調整可能に。
    *   メッセージバブルの透過率を設定画面で調整可能に。
    *   ヘッダーの色を設定画面で変更可能に。

### Version 0.3 Function Calling実装 (2024-08-12)
*   **機能追加**
